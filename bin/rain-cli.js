...
;(async () => {
  try {
    ...
    program.command('start').description('start imooc-build server').allowUnknownOption().action(startServer)
    program.command('build').description('build project by imooc-build').allowUnknownOption().action(build)
